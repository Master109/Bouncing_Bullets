PVector createPVectorForHeading(float rotation)
{
  return new PVector(cos(rotation), sin(rotation));
}

PVector createPVectorForMagnitudeAndHeading(float magnitude, float rotation)
{
  return PVector.mult(createPVectorForHeading(rotation), magnitude);
}
